...
Prometheus exemplar fetching can be enabled in time series chart settings and is disabled by default. Exemplars can be
displayed without a tracing datasource. Selecting an optional Tempo or Jaeger datasource retrieves additional trace
details lazily when an exemplar is selected.
