...
const datasourceSelectorSchema = z.union([
  z.string(),
  z.object({ kind: z.literal('LokiDatasource'), name: z.string().optional() }).strict(),
]);

export const lokiDatasourceSchema = z.object({
  kind: z.literal('LokiDatasource'),
  spec: httpDatasourceSpecSchema,
}) as unknown as PluginSchema;
export const lokiLabelNamesVariableSchema = z.object({
  kind: z.literal('LokiLabelNamesVariable'),
  spec: z
    .object({ datasource: datasourceSelectorSchema.optional(), matchers: z.array(z.string()).optional() })
    .strict(),
}) as unknown as PluginSchema;
export const lokiLabelValuesVariableSchema = z.object({
  kind: z.literal('LokiLabelValuesVariable'),
  spec: z
    .object({
...
export const lokiLogQLVariableSchema = z.object({
  kind: z.literal('LokiLogQLVariable'),
  spec: z
    .object({
      datasource: datasourceSelectorSchema.optional(),
...
export function useLokiDatasourceValidation(): void {
  ...
  useEffect(() => setSchema.current?.(lokiDatasourceSchema), []);
}

function useLokiVariableValidation(schema: PluginSchema): void {
  const validationSchemas = useContext(ValidationSchemasContext);
  const setSchema = useRef(validationSchemas?.setVariableEditorSchemaPlugin);
  useEffect(() => setSchema.current?.(schema), [schema]);
}

export function useLokiLabelNamesVariableValidation(): void {
  useLokiVariableValidation(lokiLabelNamesVariableSchema);
}

export function useLokiLabelValuesVariableValidation(): void {
  useLokiVariableValidation(lokiLabelValuesVariableSchema);
}

export function useLokiLogQLVariableValidation(): void {
  useLokiVariableValidation(lokiLogQLVariableSchema);
}
