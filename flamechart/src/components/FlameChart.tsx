...
export function FlameChart(props: FlameChartProps): ReactElement {
  ...
  const handleItemClick = useCallback((params: MouseEventsParameters<Sample>): void => {
    ...
    const functionName = sample.value[6];
    const functionId = sample.name;
  ...
  const renderItem: CustomSeriesRenderItem = useCallback(
    (_params: CustomSeriesRenderItemParams, api: CustomSeriesRenderItemAPI) => {
      const level = api.value(0);
      const start = api.coord([api.value(1), level]);
      const end = api.coord([api.value(2), level]);
      const itemHeight = (((api.size && api.size([0, 1])) || [0, 20]) as number[])[1];
      const itemWidth = (end?.[0] ?? 0) - (start?.[0] ?? 0);
      ...
      return {
        ...
        shape: {
          x: start?.[0],
          y: (start?.[1] ?? 0) - (itemHeight ?? 0) / 2,
          width: itemWidth,
          height: (itemHeight ?? ITEM_GAP) - ITEM_GAP,
          r: 0,
        },
        style: {
  ...
  const option: EChartsCoreOption = useMemo(() => {
    ...
    return option;
  }, [data, chartsTheme, theme, width, seriesData, height, renderItem]);
  ...
  const flameChart = useMemo(
    ...
    [chartsTheme.echartsTheme, handleItemClick, height, option, width],
  );
