...
const PluginKind = "ClickHouseLabelNamesVariable"
...
func create(expr string, options ...Option) (Builder, error) {
	builder := &Builder{}
	for _, opt := range append([]Option{Query(expr)}, options...) {
...
func ClickHouseLabelNames(expr string, options ...Option) listvariable.Option {
	return func(builder *listvariable.Builder) error {
		variable, err := create(expr, options...)
		if err != nil {
			return err
		}
