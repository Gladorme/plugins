...
const valueSchema = z.union([
  ...
  z.object({ value: z.string(), label: z.string().optional() }).strict(),
]);

export const staticListVariableSchema = z.object({
  kind: z.literal('StaticListVariable'),
  spec: z.object({ values: z.array(valueSchema) }).strict(),
}) as unknown as PluginSchema;

export function useStaticListVariableValidation(): void {
  ...
  useEffect(() => setSchema.current?.(staticListVariableSchema), []);
}
